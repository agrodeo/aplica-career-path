...
export function OnboardingShell({ children, trust = false, flow = false, className }: { children: ReactNode; trust?: boolean; flow?: boolean; className?: string }) {
  return (
    <div className="onboarding-scene min-h-[100svh] overflow-hidden">
      <header className="onboarding-nav">
        ...
        <nav className="flex items-center gap-5 sm:gap-7" aria-label="Principal">
          {!flow && <a className="hidden text-sm text-foreground/75 transition-colors hover:text-foreground sm:block" href="#experiencia">Cómo funciona</a>}
      ...
      {flow ? (
        <main className="onboarding-grid onboarding-grid-flow">
