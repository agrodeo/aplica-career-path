...
export function AplicaProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const saved = JSON.parse(raw) as Partial<{ onboardingStep: number; onboardingComplete: boolean; subscribed: boolean; name: string; selectedIds: string[]; savedIds: string[] }>;
  ...
  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ onboardingStep, onboardingComplete, subscribed, name, selectedIds: jobs.filter((job) => job.selected).map((job) => job.id), savedIds: jobs.filter((job) => job.saved).map((job) => job.id) }));
  }, [jobs, onboardingStep, onboardingComplete, subscribed, name]);

  const value = useMemo<AplicaState>(() => ({
    jobs, onboardingStep, onboardingComplete, subscribed, name, setName, setOnboardingStep,
