...
function JobsPage() {
  ...
  <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 backdrop-blur"><div className="mx-auto flex max-w-[1200px] items-center justify-between gap-4 px-5 py-3 md:px-8"><div><p className="text-sm font-medium">{subscribed ? selectedCount : eligible} trabajos {subscribed ? "seleccionados" : "cumplen tus criterios"}</p><p className="hidden text-xs text-muted-foreground sm:block">CV adaptado para cada puesto</p></div><Button onClick={() => subscribed ? navigate({ to: "/applications" }) : navigate({ to: "/upgrade" })}>Aplicar a {subscribed ? selectedCount : eligible} trabajos</Button></div></div>
  </div>;
}
