...
function JobsPage() {
  ...
  const selectedCount = jobs.filter((job) => job.selected && job.match >= 80).length;
  return <div className="min-h-screen bg-background"><SiteHeader /><PageShell className="pb-32">
    <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-sm font-medium text-primary">Selección actualizada hoy</p><h1 className="mt-2 text-3xl font-medium tracking-normal md:text-[40px]">{eligible} trabajos listos para aplicar</h1><p className="mt-3 text-sm text-muted-foreground">Todos pueden ser enviados automáticamente con tu perfil.</p></div><MatchInfo /></div>
    ...
    {subscribed && <div className="mt-7 flex items-center justify-between rounded-lg bg-surface p-4"><div><p className="text-sm font-medium">Elegí dónde querés aplicar</p><p className="mt-1 text-xs text-muted-foreground">Seleccionamos sólo los matches de alta calidad.</p></div></div>}
    <div className="mt-4">{filtered.length ? filtered.map((job) => <JobRow key={job.id} job={job} selectable={subscribed} />) : <div className="py-24 text-center"><BriefcaseBusiness className="mx-auto h-8 w-8 text-muted-foreground"/><h2 className="mt-5 text-xl font-medium">Todavía no encontramos suficientes trabajos que encajen con tus criterios.</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-muted-foreground">Probá ajustando ubicación, salario, modalidad, seniority o industria.</p><Button className="mt-6" variant="outline" onClick={() => { setMin(70); setQuery(""); }}>Ajustar búsqueda</Button></div>}</div>
  </PageShell>
  <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 backdrop-blur"><div className="mx-auto flex max-w-[1200px] items-center justify-between gap-4 px-5 py-3 md:px-8"><div><p className="text-sm font-medium">{subscribed ? selectedCount : eligible} trabajos {subscribed ? "seleccionados" : "listos para aplicar"}</p><p className="hidden text-xs text-muted-foreground sm:block">CV adaptado para cada puesto</p></div><Button onClick={() => subscribed ? navigate({ to: "/applications" }) : navigate({ to: "/upgrade" })}>Aplicar a los {subscribed ? selectedCount : eligible}</Button></div></div>
  </div>;
}
